...
while ($row = mysqli_fetch_assoc($re)) {
    ...
    // Check if 'fname' and 'lname' are valid strings and not arrays or other types
    $fname = $row['fname'];
    $lname = $row['lname'];

    // Debugging log to help identify the issue (can be removed later)
    // You can echo this out to check the exact value types:
    // echo "<pre>"; print_r($row['fname']); echo "</pre>";

    // Ensure fname and lname are treated as strings, fallback to 'N/A' if invalid
    $fname = is_string($fname) ? $fname : 'N/A';
    $lname = is_string($lname) ? $lname : 'N/A';

    // Assign default values if they are empty
    $fname = !empty($fname) ? $fname : 'N/A';
    $lname = !empty($lname) ? $lname : 'N/A';
